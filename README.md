## Hello, I'm Yassi

I build products and tools. Currently Building the control room for Django.

Member, Django Software Foundation

### Writing
- [https://yassi.dev](https://yassi.dev)
- Build a site with the same theme using [hugo-theme-yassi](https://github.com/yassi/hugo-theme-yassi)

### Django Control Room
Create a command center out of your Django admin.

* Official website: [Django Control Room](https://djangocontrolroom.com)

| Repo | Description | Stars |
| --- | --- | --- |
| [dj-control-room](https://github.com/yassi/dj-control-room) | Build a control center in your admin with different tools and panels. The missing plugin system for the Django Admin. | [![GitHub Repo stars](https://img.shields.io/github/stars/yassi/dj-control-room?style=social)](https://github.com/yassi/dj-control-room/stargazers) |

#### Panels Available
| Panel | What it does |
| --- | --- |
| [dj-control-room-base](https://github.com/yassi/dj-control-room-base) | Core library and example panel / design system |
| [dj-redis-panel](https://github.com/yassi/dj-redis-panel) | Redis visibility inside Django admin |
| [dj-cache-panel](https://github.com/yassi/dj-cache-panel) | Inspect and debug your cache layer |
| [dj-celery-panel](https://github.com/yassi/dj-celery-panel) | Monitor workers and tasks from admin |
| [dj-urls-panel](https://github.com/yassi/dj-urls-panel) | Visualize and test Django URL routing inside the Django Admin |
| [dj-signals-panel](https://github.com/yassi/dj-signals-panel) | See all your registered signals and recievers in the Django admin. |

More coming soon.

Build your own panels:

- [cookiecutter-dj-control-room-plugin](https://github.com/yassi/cookiecutter-dj-control-room-plugin)
- [Build your first panel tutorial](https://djangocontrolroom.com/guides/create-django-control-room-panel)

### Other Projects

| Project | Description |
| --- | --- |
| [Pi Patterns](https://pi.yassi.dev/) | Visualize sub sequences of pi and generate amazing art |


### Support

If Django Control Room saves your team time in production,
you can support its ongoing development here:

[![Sponsor](https://img.shields.io/badge/Sponsor-GitHub-pink?logo=github)](https://github.com/sponsors/yassi)
